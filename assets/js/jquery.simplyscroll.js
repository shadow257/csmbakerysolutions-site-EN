    ...
    $.simplyScroll.fn.extend({
        ...
        init: function() {
            ...
            if (!this.isHorizontal) {
                ...
                this.clipMax = this.$items.length * this.itemMax;
            ...
            if (this.isAuto && this.o.autoMode == 'loop') {
				...
				//if(this.clipMax > 1000){

				var ww = $(window).width();	

				//if(this.$items.length > 7){

				//console.log(ww + "-" + this.clipMax);

				if((this.clipMax) > ww){

					var items_append = this.$items.slice(0, addItems).clone(true).appendTo(this.$list);

					items_append = this.$items.slice(0, addItems).clone(true).appendTo(this.$list);

					var items_prepend = this.$items.slice(-addItems).clone(true).prependTo(this.$list);

					items_prepend = this.$items.slice(-addItems).clone(true).prependTo(this.$list);

					this.resetPositionForwards = this.resetPosition = addItems * this.itemMax;

					this.resetPositionBackwards = this.$items.length * this.itemMax;

					this.$btnBack.css('display','block');

            		this.$btnForward.css('display','block');

				}

				else{

					this.$btnBack.css('display','none');

            		this.$btnForward.css('display','none');
